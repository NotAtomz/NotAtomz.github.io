...
class RuntimeSignal {
    ...
    _startListener(listener, args) {
        ...
        const task = this.runtime.scheduler.createTaskFromCallable(listener.callback, args, {
            ...
            silentErrors: true,
            onError: error => {
                const signature=String(error?.message||error||"runtime signal error");
                if(listener.__lastErrorSignature===signature) listener.__errorCount=(listener.__errorCount||0)+1;
                else { listener.__lastErrorSignature=signature; listener.__errorCount=1; }

                if(listener.__errorCount===1) console.error(`[Luau] ${this.Name}:`, error);

                if(listener.__errorCount>=3) {
                    listener.connected=false;
                    listener.pendingArgs=null;
                    if(listener.connection) listener.connection.Connected=false;
                    this.listeners.delete(listener);
                    console.warn(`[Luau] Disconnected failing ${this.Name} listener after 3 identical errors.`);
                }
            },
            onComplete: (_result,error) => {
                listener.running = false;
                listener.task = null;
                if(!error) { listener.__errorCount=0; listener.__lastErrorSignature=""; }
...
class InterpreterRuntime {
    ...
    *callValue(callee,args,node=null) {
    ...
    dispatchExternal(callback,args=[],label="Lua callback") {
        ...
        const task=this.scheduler.createTaskFromCallable(callback,args,{label,silentErrors:true});
