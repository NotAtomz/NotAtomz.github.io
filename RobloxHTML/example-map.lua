...
block.Position = Vector3.new(-15, 15, 0)
...
-- Classic leaderboard
Players:AddLeaderboardKey("KOs")
Players:AddLeaderboardKey("Wipeouts")
...
local wipes = Instance.new("IntValue")
wipes.Name = "Wipeouts"
wipes.Value = 3
wipes.Parent = player
...
-- Put it directly in the Backpack so these exact event
-- connections stay attached to the item being used.
...
local function stopDragging()
    ...
    if selectedPart then
        ...
        -- Stop any velocity gained while being moved.
        selectedPart.Velocity =
            Vector3.new(0, 0, 0)

        selectedPart.RotVelocity =
            Vector3.new(0, 0, 0)

        -- Restore however the Part was before dragging.
        selectedPart.Anchored =
            oldAnchored
    end
...
local function beginDragging()
    ...
    -- Don't grab your own character.
    if player.Character and
       target:IsDescendantOf(player.Character) then
        return
    end
    ...
    -- Temporarily anchor it so gravity/physics does not
    -- fight against the mouse while dragging.
    selectedPart.Anchored =
        true
...
moveTool.Selected:Connect(function(selectedMouse)
...
moveTool.Deselected:Connect(function()
...
RunService.RenderStepped:Connect(function(deltaTime)
    ...
    -- Get how far the mouse moved since last frame.
    local deltaX =
        mouse.X - lastMouseX
    ...
    -- Distance from camera to Part.
    -- Further Parts need more world movement per pixel.
    local distance =
        (
    ...
    -- Mouse sensitivity based on depth.
    local moveScale =
        math.max(distance, 5) * 0.0025
    ...
    -- Keep physics completely still while held.
    selectedPart.Velocity =
        Vector3.new(0, 0, 0)
